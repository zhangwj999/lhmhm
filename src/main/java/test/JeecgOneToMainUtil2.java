...
public class JeecgOneToMainUtil2 {
	...
	public static void main(String[] args) {
		...
		codeParamEntityIn.setTableName("office");//主表[表名]
		codeParamEntityIn.setEntityName("Office");	 //主表[实体名]
		codeParamEntityIn.setEntityPackage("office");	 //主表[包名]
		codeParamEntityIn.setFtlDescription("理财室");	 //主表[描述]
		
		//第二步：设置子表集合
		List<SubTableEntity> subTabParamIn = new ArrayList<SubTableEntity>();
		//[1].子表一
		SubTableEntity po = new SubTableEntity();
		po.setTableName("office_channel");//子表[表名]
		po.setEntityName("OfficeChannel");	 //子表[实体名]
		po.setEntityPackage("office");			 //子表[包]
		po.setFtlDescription("通道");		 //子表[描述]
		//子表[外键:与主表关联外键]
		//说明：这里面的外键是子表的外键字段,非主表和子表的对应关系  GORDER_ID修改为ID
		po.setForeignKeys(new String[]{"ID","OFFICE_ID"});
		...
		//po2.setEntityPackage("office"); 					//子表[包]
