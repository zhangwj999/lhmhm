  ...
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="channelController.do?save">
			...
			<tr>
				...
				<td align="right">
					...
					<td class="value">
						...
						<c:forEach items="${officesList}" var="terminal">
							<option value="${terminal.fid }" <c:if test="${channelPage.officeId==terminal.fid}">selected="selected"</c:if>>${terminal.fname}</option>
						</c:forEach>
					</select> 
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="value">
					<t:dictSelect field="type" typeGroupCode="chantype"
                		defaultVal="${channelPage.type}" hasLabel="false" title="性别(没有label)"></t:dictSelect>
